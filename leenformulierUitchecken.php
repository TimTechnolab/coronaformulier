...
$sqlGeleend = $conn->query("SELECT laptopnaam FROM laptop WHERE status = 'geleend'");
...
if(isset($_POST['submit'])){
    ...
    if(isset($emailCheck)){
        $sqlUpdate = "UPDATE laptop SET status = 'beschikbaar' WHERE laptopnaam = '$laptop'";
    if(mysqli_query($con, $sqlUpdate)){
                echo "<script type='text/javascript'>alert('Laptop is succesvol ingeleverd.')</script>";
...
<html>
    <head>
        <title>Technolab Leiden | LeenFormulier uitchecken</title>
        <meta charset="UTF-8">
...
<body>
                        ...
                        <nav class="navbar navbar-light shadow d-flex justify-content-between" style="background-color: white;">
                            <div class="dropdown">
                                ...
                                <a class="navbar-brand" href="">
                                    ...
                                    <a class="btn btn-outline-success my-2 my-sm-0" href="leenformulierInchecken.php">Inchecken</a>   
                        </nav>
                    </div>
                </div>
        </div>
    ...
    <!-- Dit is het formulier zelf, dus waar je alles invult in de browser -->
        <form method="POST" id="MainForm">
            <h1>Leen formulier uitchecken</h1>
                ...
                <div class="d-flex flex-column" placeholder="Functie">
                    <select class="p-1" name="laptop" id="laptop" required>
                        <option></option>
                        <?php while($row = $sqlGeleend->fetch_assoc()){?>
                        <option value="<?=$row['laptopnaam']?>"><?=$row['laptopnaam']?></option>
                        <?php } ?>
                    </select>
                <div class="d-flex justify-content-center"> 
                    <button class="SubmitButtonForm" type="submit" name="submit">Uitchecken</button>
                </div>
            </div>
        </form>
    </main>
